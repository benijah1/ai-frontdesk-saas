...
/** Keep the header/nav in app/layout.tsx only to avoid double menus. */
type Trade = "General" | "HVAC" | "Plumbing" | "Roofing" | "Electrical" | "Solar";
...
export default function Home() {
  ...
  const tradeTagline: Record<Trade, string> = {
    General:
      "We answer calls, texts, and website chats in seconds, qualify the lead, quote basics, and book the job into your calendar.",
    HVAC:
      "We qualify AC/heat issues, collect model photos, suggest windows, and book tune-ups or diagnostics directly into your calendar.",
    Plumbing:
      "We triage leaks/clogs, capture fixture photos, give travel+diag estimates, and book visits into your schedule.",
    Roofing:
      "We intake roof size, material, leak photos, storm details, and schedule inspections with homeowner confirmations.",
    Electrical:
      "We scope panels/outlet counts, detect emergencies, and schedule estimates or same-day dispatch when needed.",
    Solar:
      "We pre-qualify usage, roof fit, shade, and incentives, then schedule a design consult without back-and-forth.",
  };
  ...
  const tradeFeatures: Record<Trade, { title: string; desc: string }[]> = {
  ...
  const stepsByTrade: Record<Trade, { step: string; title: string; text: string }[]> = {
    General: [
  ...
  const plans = [
    {
      ...
      bullets: ["Up to 200 conversations/mo", "1 business line + SMS", "Website chat widget", "Leads to email + CRM"],
    },
    {
      name: "Pro",
      price: "$499",
      ...
      badge: "Most popular",
      highlight: true,
      bullets: ["Up to 800 conversations/mo", "Booking into Google/Outlook", "After-hours + weekends", "Call recording + QA scores"],
    },
    {
      name: "Scale",
      price: "Custom",
      period: "",
      badge: "High-volume shops",
      bullets: ["Unlimited conversations", "Multi-location routing", "Job-software integration", "Dedicated success manager"],
    },
  ];
  ...
  const testimonials = [
    {
      quote:
        "We went from missing 30% of calls to booking 18 more jobs in the first month. Nights and Sundays are finally covered.",
      name: "Maria G.",
  ...
  const dynamicFeatures = [
    ...
    ...(trade === "General"
      ? tradeFeatures.General
      : [{ title: tradeFeatures[trade][0].title, desc: tradeFeatures[trade][0].desc }]),
  ...
  return (
    <main className="min-h-screen scroll-smooth bg-gradient-to-b from-slate-950 via-slate-950 to-slate-900 text-slate-100">
      {/* HERO (no header here; header stays in app/layout.tsx) */}
        ...
        <div className="mx-auto max-w-screen-xl px-4 py-16 md:py-20">
          <div className="grid items-center gap-10 lg:grid-cols-2">
            <div>
                ...
                {(["General", "HVAC", "Plumbing", "Roofing", "Electrical", "Solar"] as const).map((t) => (
                  <button
                    ...
                    className={`rounded-full border px-3 py-1 text-xs ${
                      trade === t ? "border-indigo-300 bg-indigo-500/20" : "border-white/10 bg-white/5 hover:bg-white/10"
                    }`}
                  >
                    {t}
                  </button>
              ...
              <p className="mt-5 text-lg text-white/80">{tradeTagline[trade]}</p>
              <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                <button
                  onClick={() => scrollTo("get-demo")}
                  className="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-500 px-5 py-3 font-medium hover:bg-indigo-400"
                >
                  Book a live demo <CalendarIcon />
                </button>
                <button
                  onClick={() => scrollTo("pricing")}
                  className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/10 px-5 py-3 font-medium hover:bg-white/5"
                >
              ...
              <div className="mt-6 flex flex-wrap items-center gap-6 text-sm text-white/60">
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="h-4 w-4" /> 2-second pick-up
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="h-4 w-4" /> After-hours covered
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="h-4 w-4" /> No long contracts
                </div>
              </div>
            </div>
        ...
        <div className="mx-auto max-w-screen-xl px-4 py-10">
          ...
          <div className="mt-6 grid grid-cols-2 gap-6 opacity-80 sm:grid-cols-3 md:grid-cols-6">
            {["Google Calendar", "Outlook", "Jobber", "ServiceTitan", "Housecall Pro", "Zapier"].map((name) => (
              <div
                key={name}
                className="flex items-center justify-center rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-xs"
              >
                {name}
              </div>
            ))}
            ...
            {dynamicFeatures.map((f) => (
              <div key={f.title} className="rounded-2xl border border-white/10 bg-white/5 p-6">
                <div className="inline-flex items-center justify-center rounded-xl bg-white/10 p-2">
                  {"icon" in f ? (f as any).icon : <Wrench className="h-6 w-6" />}
                </div>
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-3">
            {stepsByTrade[trade].map((s) => (
              <div key={s.step} className="rounded-2xl border border-white/10 bg-white/5 p-6">
                <div className="text-sm font-semibold text-indigo-300">{s.step}</div>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-indigo-500/20 to-transparent p-6">
              <h3 className="flex items-center gap-2 text-xl font-semibold">
                <Headset className="h-5 w-5" /> Live transfer when it matters
              </h3>
              <p className="mt-2 text-sm text-white/80">
                Emergency? High-ticket roof? We detect urgency and warm-transfer to your on-call in seconds.
              </p>
              <ul className="mt-4 space-y-2 text-sm text-white/80">
                {[
                  "Smart triage & escalation",
                  "Photo/video intake via SMS",
                  "Detailed CRM notes + call links",
                  "Follow-up reminders if no show",
                ].map((li) => (
                  <li key={li} className="flex items-start gap-2">
                    <CheckCircle2 className="mt-0.5 h-4 w-4" /> {li}
                  </li>
                ))}
              </ul>
            </div>
            ...
            {plans.map((p) => (
              <div
                ...
                className={`rounded-2xl border p-6 ${
                  (p as any).highlight ? "border-indigo-400 bg-indigo-500/10" : "border-white/10 bg-white/5"
                }`}
              >
                ...
                <ul className="mt-4 space-y-2 text-sm text-white/80">
                  {p.bullets.map((b) => (
                    <li key={b} className="flex items-start gap-2">
                      <CheckCircle2 className="mt-0.5 h-4 w-4" /> {b}
                    </li>
                  ))}
                </ul>
                <button
                  onClick={() => scrollTo("get-demo")}
                  className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-500 px-4 py-2 font-medium hover:bg-indigo-400"
                >
                  Choose {p.name}
                </button>
              </div>
      ...
      <section id="get-demo" className="relative scroll-mt-24">
        <div className="mx-auto max-w-screen-xl px-4 py-20">
          <div className="grid items-start gap-8 lg:grid-cols-2">
            <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
              ...
              <form onSubmit={handleSubmit} className="mt-4 flex flex-col gap-3 sm:flex-row">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-500 px-5 py-3 font-medium hover:bg-indigo-400 disabled:opacity-60"
                >
            ...
            <div id="faq" className="scroll-mt-24 rounded-2xl border border-white/10 bg-white/5 p-6">
              <h3 className="flex items-center gap-2 text-2xl font-bold">
                <Users className="h-5 w-5" /> Frequently asked
              </h3>
              <ul className="mt-4 space-y-4">
                {[
                  { q: "Will it sound like a robot?", a: "No. It uses natural prosody and trades vocabulary. Most callers assume it’s a friendly dispatcher." },
                  { q: "How do bookings appear?", a: "Events land on your calendar with full notes, call link, photos, and homeowner contact details." },
                  { q: "Can I keep my number?", a: "Yes. We can port or simply forward — your number stays the same." },
                  { q: "What about emergencies?", a: "You set rules. We escalate and live-transfer to on-call when certain keywords or conditions are met." },
                ].map((item) => (
        ...
        <div className="mx-auto max-w-screen-xl px-4 py-10 text-sm text-white/70">
          <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
            ...
            <div className="flex items-center gap-6">
              <button onClick={() => scrollTo("pricing")} className="hover:text-white">
                Pricing
              </button>
...
function HeroMockup({ trade }: { trade: Trade }) {
  const firstLine =
    trade === "HVAC"
      ? "Homeowner: “AC stopped cooling and it’s 88°F.”"
      : trade === "Plumbing"
      ? "Homeowner: “There’s a leak under the kitchen sink.”"
      : trade === "Roofing"
      ? "Homeowner: “Ceiling stain after last night’s storm.”"
      : trade === "Electrical"
      ? "Homeowner: “Half the house lost power and breakers keep tripping.”"
      : trade === "Solar"
      ? "Homeowner: “Interested in lowering my bill with solar — what’s the process?”"
      : "Homeowner: “I need a service appointment.”";
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <div className="rounded-xl border border-white/10 bg-slate-950/60 p-4">
          <div className="flex items-center gap-2 text-sm font-semibold">
            <Phone className="h-4 w-4" /> Incoming Call
          </div>
        ...
        <div className="rounded-xl border border-white/10 bg-slate-950/60 p-4">
          <div className="flex items-center gap-2 text-sm font-semibold">
            <CalendarGlyph /> Booking Created
          </div>
          <ul className="mt-3 space-y-2 text-sm text-white/80">
            <li className="flex items-start gap-2">
              <CheckCircle2 className="mt-0.5 h-4 w-4" /> Calendar event with notes
            </li>
            <li className="flex items-start gap-2">
              <CheckCircle2 className="mt-0.5 h-4 w-4" /> SMS confirmation to homeowner
            </li>
            <li className="flex items-start gap-2">
              <CheckCircle2 className="mt-0.5 h-4 w-4" /> Job details synced to your CRM
            </li>
          </ul>
        </div>
      </div>
...
function CalendarGlyph() {
  return (
    <span className="inline-flex items-center gap-2">
      <CalendarIconDot /> <span>AI Calendar</span>
    </span>
  );
}
